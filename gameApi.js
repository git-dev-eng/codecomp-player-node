...
var auth = "Basic " + new Buffer(config.team + ":" + config.password).toString("base64");
...
function joinGame(callback){
    ...
    request.post({
        ...
        if(response && response.statusCode >=200 && response.statusCode < 300){
            ...
            callback({ errorResponse : (response) ? response.body : 'No response' }, null);
...
module.exports = { join : joinGame, status : getGameStatus, guess : guess }